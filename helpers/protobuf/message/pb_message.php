...
abstract class PBMessage
{
    ...
    public function SerializeToString($rec=-1)
    {
        ...
        foreach ($this->fields as $index => $field)
        {
            if (is_array($this->values[$index]) && count($this->values[$index]) > 0)
            {
                ...
                foreach ($this->values[$index] as $i=>$array)
                {
                	if (!is_object($array)) {
                		$array = new $this->fields[$index]();
                		$array->value = $this->values[$index][$i];
                	}
            ...
            else if (is_object($this->values[$index]))
            {
                // wired and type
                $newstring = '';
            	...
            	$obj_val = new $this->fields[$index]();
            	$obj_val->value = $this->values[$index];
            	$this->values[$index] = $obj_val;
     ...
     * @return string the encoded message
     */
    public function toArray ($rec=-1)
    {
    	$res = array();
 
    	if (isset($this->fields_name)) {
	    	foreach ($this->fields_name as $index => $name)
	    	{
	    		if (is_array($this->values[$index]))
	    		{
	    			$res[$name] = array();
	    			// make serialization for every array
	    			foreach ($this->values[$index] as $i=>$value)
	    			{
	    				if (!is_object($value)) {
	    					$value = new $this->fields[$index]();
	    					$value->value = $this->values[$index][$i];
	    				}
	    				$res[$name][] = $value->toArray($index);
	    
	    			}
	    		}
	    		else if (is_object($this->values[$index]))
	    		{
	    			$res[$name] = $this->values[$index]->toArray($index);
     ...
     * @return string the encoded message
     */
    public function toObject ($rec=-1)
    {
    	$res = new stdClass();
    		...
    		foreach ($this->fields_name as $index => $name)
    		{
    			if (is_array($this->values[$index]))
    			{
    				...
    				foreach ($this->values[$index] as $i=>$value)
    				{
    					...
    					array_push($res->$name, $value->toObject($index));
    					 
    				}
    			}
    			else if (is_object($this->values[$index]))
    			{
    				$res->$name = $this->values[$index]->toObject($index);
