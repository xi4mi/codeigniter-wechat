...
class PBParser
{
    ...
    // different types
    var $scalar_types = array('double'=>'PBInt', 'float', 'int32' => 'PBInt', 'int64' => 'PBInt',
    ...
    private function _parse_file ($protofile)
    {
		...
		if (isset($this->f_parsed[$filename])) {
			return;
		} else {
			$this->f_parsed[$filename] = 1;
		}
        ...
        $string = trim($string);
        $imports = $this->_parse_message_head($string, '');
        foreach ($imports as $k=>$file) {
            $file = $file[0]!='.'? $dirname.'/'.$file : $file;
            unset($this->import[$k]);
    ...
    private function _create_class_file($filename)
    {
        ...
        foreach ($this->m_types as $classfile)
        {
            ...
            if ($classfile['type'] == 'message')
            {
				...
				$string .= "\tpublic \$name = '" . $this->ns . '.' . $classname  . "';\n";
    ...
    private function _create_class_body($classfile, &$string, $classname)
    {
        foreach($classfile as $field)
        {
            ...
            else if (isset($field['value']['repeated']))
            {
                ...
                $string .= '    $this->_add_arr_value("' . $field['value']['value'] . '");'  . "\n";
                $string .= '    return $this;' . "\n";
                $string .= "  }\n";
            ...
            else
            {
                ...
                $string .= '    $this->_set_value("' . $field['value']['value'] . '", $value);'  . "\n";
                $string .= '    return $this;' . "\n";
                $string .= "  }\n";
            }
        }
     ...
     */
    private function _parse_message_head(&$string, $m_name, $path = '')
    {
        $myarray = $imports = array();
        $string = trim($string);
        if ($string == '')
            return array();
        ...
        while (strlen($string) > 0)
        {
            ...
            if (strtolower($next) == 'package')
            {
                ...
                $ns = $this->_next($string);
                $this->ns = (substr($ns, -1)==';'? substr($ns, 0, -1) : $ns);
				
				$offset = $this->_get_begin_end($string, $this->ns, ";");
				$string = '' . trim(substr($string, $offset['end']));
            }
            else if (strtolower($next) == 'import')
            {
                $string = trim(substr($string, strlen($next)));
                $file = $this->_next($string);
                $file = (substr($file, -1)==';'? substr($file, 0, -1) : $file);
                $file = trim($file, '"');
                $imports[] = $file;

				$offset = $this->_get_begin_end($string, $file, ";");
				$string = '' . trim(substr($string, $offset['end']));
